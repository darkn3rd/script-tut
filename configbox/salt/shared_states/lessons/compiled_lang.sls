{#- compiled_lang.sls - see ./gen_scripts.sls's own header comment;
...
{%- set steps = lessons.areas.get('compiled_lang', {}).get('steps', []) %}
...
{%- for step in steps if step['type'] != 'salt_formula' %}
{{ step_lib.install_step('lessons-compiled_lang-' ~ loop.index0, step, lessons.user, home) }}
{%- endfor %}
