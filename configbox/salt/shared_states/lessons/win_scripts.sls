{#- win_scripts.sls - see ./gen_scripts.sls's own header comment; same
  shape, different area key. #}

{%- from "lessons/map.jinja" import lessons, home with context %}
{%- import "lessons/install_step.jinja" as step_lib with context %}
{%- set steps = lessons.areas.get('win_scripts', {}).get('steps', []) %}
...
{%- for step in steps if step['type'] != 'salt_formula' %}
{{ step_lib.install_step('lessons-win_scripts-' ~ loop.index0, step, lessons.user, home) }}
{%- endfor %}
