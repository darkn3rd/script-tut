...
4. If the scripting language supports this, utilize variable interpolation, that is, the variable is embedded within the string.  
...
3. Write a script that uses floating point values and performs operations on the values without losing precision.  In other words, if you divide 3 by 2, the value should be 1.5, not 1.
...
5. Write a script that uses some advanced mathematical operation, such as calculating cosine.  This will undoubtedly utilize some form of mathematical library.
...
2. The test should evaluate one of three different states.
...
2. Utilize alternative looping constructs provided by the scripting language to achieve the same results.
...
3. Utilize various looping constructs afforded in the scripting language to achieve the same results.
4. As a bonus, create a spin loop that breaks out of loop upon the user requests.


### Collection Loop

1. Write a script that can process a list of items, such as a directory listing, a space delimited string, or an array.
2. This should use the scripting languages native collection loop, such as ```for ... in``` or ```for each```.  Note that some scripting languages may not have this functionality, so an iterative loop (above) must be used.
...
2. Create a script that creates an associative array by using a supplied list of key/value pairs, then prints the key and value pairs.
3. Create a script that merges two associative arrays together, and then prints the key and value pairs.

# Basic-Intermediate Scripting

The basic-intermediate section covers processing basic command-line arguments, organizing or packaging code into subroutines, using functions that return values, and passing parameters to subroutines or functions.
...
3. Create a script that prints out all the arguments passed to the script, in reverse order.
