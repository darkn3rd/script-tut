#!/usr/bin/env ksh
# mksh's associative-array support is unreliable (see h00/h10's own
#  notes) - each drink's quantity is tracked as a separate plain
#  variable instead of a hash, and the final list is just built by
#  iterating them in a hardcoded alphabetical-by-key order rather than
#  sorting dynamically.
if [ $# -eq 0 ]; then
  q_coffee=$((RANDOM % 3))
  q_espresso=$((RANDOM % 3))
...
#  script is about to exit. Explicit /usr/bin/env, not bare "env": this
